...
async def judgeUuid(uuid: int) -> tuple:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            data_json = await resp.json()
            return (True,
                    data_json["data"]["name"]) if data_json["code"] == 0 else (False,
                                                                               data_json["message"])
...
async def bilibili_subscribe_auth_listener(app: GraiaMiraiApplication, group: Group,
                                           message: MessageChain):
    if message.asDisplay().startswith("订阅") and len(message.asDisplay().split(' ')) == 2:
        try:
    ...
    if message.asDisplay().startswith("取消订阅") and len(message.asDisplay().split(' ')) == 2:
        try:
            uuid = int(message.asDisplay().split(' ')[1])
            res = await judgeUuid(uuid)
            msg = MessageChain.create([Plain(res[1])])
