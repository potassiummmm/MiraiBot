...
def get_riddler_text():
    content_list = [
        "关于这个事，我简单说两句，至于我的身份，你明白就行。", "总而言之，这个事呢，现在就是这个情况，具体的呢，大家也都看得到。",
        "我因为这个身份上的问题，也得出来说那么几句。", "可能，你听的不是很明白，但是意思就是那么个意思。",
        "我的身份呢，不知道的你也不用去猜,这种事情见得多了，我只想说懂得都懂，不懂的我也不多解释，毕竟自己知道就好。",
        ...
        "懂了就能收割不懂的，你甚至都不知道自己不懂。只是在有些时候，某些人对某些事情不懂装懂，还以为别人不懂。",
    ...
    return content_list[random_seq[0]] + content_list[
        random_seq[1]] + content_list[random_seq[2]]
...
async def group_message_listener(app: GraiaMiraiApplication, group: Group,
    ...
    if message.asDisplay().startswith("营销号"):
        ...
        await app.sendGroupMessage(
            group,
            MessageChain.create([Plain(get_yxh_text(msg[1], msg[2], msg[3]))]))
    if message.asDisplay().startswith("谜语人") or message.asDisplay().startswith(
            "猜谜"):
        await app.sendGroupMessage(
            group, MessageChain.create([Plain(get_riddler_text())]))
